...
if [ $# != 6 ]; then
   ...
   echo "This version applies an connLM and mixes it with the LM scores"
   echo "previously in the lattices., controlled by the first parameter (connlm-weight)"
   echo ""
   echo "Usage: utils/connlmrescore.sh <connlm-weight> <old-lang-dir> <connlm-dir> <data-dir> <input-decode-dir> <output-decode-dir>"
   echo "Main options:"
...
for f in $connlm_dir/final.clm $data/feats.scp $indir/lat.1.gz; do
...
if [ $stage -le 6 ]; then
  echo "$0: invoking connlm_compute_scores.sh which calls get connLM scores."
  for j in `seq $nj`; do
  $cmd JOB=$j:$j $dir/log/connlm_compute_scores.JOB.log \
    utils/connlm_compute_scores.sh $connlm_dir $adir.JOB/temp $adir.JOB/words_text $adir.JOB/lmwt.clm \
    || exit 1;
  done
fi
if [ $stage -le 7 ]; then
  echo "$0: reconstructing total LM+graph scores including interpolation of connLM and old LM scores."
  for n in `seq $nj`; do
    paste $adir.$n/lmwt.nolm $adir.$n/lmwt.lmonly $adir.$n/lmwt.clm | awk -v weight=$connlm_weight \
      '{ key=$1; graphscore=$2; lmscore=$4; connlmscore=$6;
     score = graphscore+(weight*connlmscore)+((1-weight)*lmscore);
     print $1,score; } ' > $adir.$n/lmwt.interp.$connlm_weight || exit 1;
  done
fi
...
if [ $stage -le 8 ]; then
  ...
  $cmd JOB=1:$nj $dir/log/reconstruct_lattice.JOB.log \
    ...
    "ark:$adir.JOB/lmwt.interp.$connlm_weight" "ark:$adir.JOB/acwt" ark:- \| \
    nbest-to-lattice ark:- "ark:|gzip -c >$dir/lat.JOB.gz" || exit 1;
fi
